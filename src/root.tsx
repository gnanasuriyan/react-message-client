...
function Root() {
  ...
  const { changed } = useTreeChanges(loggedInUser);

  const { isAuthenticated, user } = loggedInUser;

  useEffect(() => {
    if (changed('isAuthenticated', true)) {
      dispatch(alertShow(`Hello! And welcome ${user.username}!`, { type: 'success', icon: 'bell', timeout: 10 }));
    }
  }, [dispatch, changed]);
        ...
        <AppWrapper data-component-name="app">
          ...
          <Main isAuthenticated={isAuthenticated}>
            <Routes>
              <Route
                element={
                  <PublicRoute isAuthenticated={isAuthenticated} to="/messages">
                    <Login />
                  </PublicRoute>
                }
                path="/"
